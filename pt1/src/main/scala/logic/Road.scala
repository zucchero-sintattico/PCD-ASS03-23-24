        ...
        case p: ProcessStep =>
          context.spawnAnonymous(
            Aggregator[CarAction, ProcessStep](
              sendRequests = replyTo => for cars <- p.cars do cars.foreach(c => c.carRef ! CarActor.DecideAction(p.dt, Road.calculateCarPerception(c, cars), replyTo)),
              expectedReplies = carActors.size,
              replyTo = context.self,
...
object Road:
  //is apply implicit???
  def calculateCarPerception(car: CarRecord, cars: List[CarRecord]): CarPerception =
    CarPerception(car.carRecord.position, findNearestCarInFront(car, cars), Option.empty)

  private def findNearestCarInFront(car: CarRecord, cars: List[CarRecord]): Option[Car] =
    cars.map(_.carRecord).filter(_.position > car.carRecord.position).sortBy(_.position).headOption
